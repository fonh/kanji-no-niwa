...
| 2    | レッスン (Leçons)  | **Carnet de leçons** *(redéfini 2026-07-07, audit 06 — remplace « Batch de leçons actif + historique » : le mot « batch » n'était défini nulle part et désignait trois choses différentes selon les docs, purgé partout)* : table des matières du livre de leçons, par zone — voir § Leçons, « Carnet de leçons » |
...
| Radio     | Émissions d'Oak, radio d'ambiance. **Écoute adoptée à deux formats (2026-07-06, revue pédagogique) :** court quotidien — le mot de passe de Buena (11h00) devient un vrai exercice d'écoute (entendre → retenir → restituer au studio 2F) ; long hebdomadaire (#7) — émission d'Oak de 2-3 min avec quiz optionnel, courbe N5→N1, disponible post-Tour Radio. **Tableau du jour (adopté 2026-07-06, I-12)** : l'onglet annonce chaque matin le contenu calendaire réel (frère/sœur du jour, concours du Parc mar/jeu/sam, photographe, mercredi sec du Lac Colère) — façon diégétique de surfacer les `time_window`, comme la radio d'HGSS. Designs à la synthèse. |
| Grammaire *(remplace Textes, 2026-07-01)* | Glossaire des points de grammaire rencontrés (`grammar_encounters`) — formation, exemple, note, tirés de la même entrée de la **table `grammar`** que les leçons (format grammar_note — rattachement précisé 2026-07-07, audit 06, finding 06-A3). Comble un manque : cette donnée existait déjà en base sans aucune vue joueur. |
...
| ----- | ---------------------------------------------------------------- | -------------------------------------------------------------------------- |
| 飛 (Vol) | La femme de Chuck, à Irisia, après la victoire sur Chuck | Voyage rapide — tap sur n'importe quelle ville visitée pour s'y téléporter, **intra-région uniquement** *(adopté 2026-07-06, I-1 : les liaisons Johto↔Kanto restent le SS Aqua et le Magnet Train — comme en HGSS, où Vol ne franchit pas les régions — pour que bateau et train gardent leur valeur)* |
...
Tous les messages de Fukuda stockés dans `/content/dialogues/fukuda/` en markdown, indexés par `event_type` et `event_ref` *(renommés 2026-07-06, audit 04, finding 04-D2 — collision de nom avec le `trigger_type` de `map_npcs`)*. Aucune génération à runtime.
...
| `srs_cards`          | user_id, item_type (kanji \| word), item_id, facet (sens \| lecture), due_date, stability, difficulty (champs FSRS) *(corrigé 2026-07-06, audit 03, finding 03-B1 : l'ancien schéma `card_type` seul ne référençait aucun contenu et ne distinguait pas les 2 cartes indépendantes par kanji/mot — 4 variantes à représenter : kanji-sens, kanji-lecture, mot-sens, mot-lecture)*                                                                                                                                                      |
...
| `text_completions`   | user_id, text_id, score *(= % de questions réussies en **première tentative** — seule définition qui ait un sens sous retry-jusqu'à-correct ; statistique partout, ne gate que le combat Red ; précisé 2026-07-07, audit 05, finding 05-A1)*, completed_at                                                                                                                                                                                  |
