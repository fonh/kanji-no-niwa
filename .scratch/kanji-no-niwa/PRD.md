...
N propre à chaque CS et croissant, calibré à la synthèse à ~50-60 % du corpus atteignable sans CS à ce
moment *(révisé 2026-07-06, repasse progression — remplace « tous les textes des zones débloquées lus »
...
**Disposition** utilise des phrases Tatoeba pré-tokenisées avec kuromoji au build time. Les chunks sont stockés comme `chunks[]` en base. Le client mélange et valide par comparaison de séquence de `surface_form`. Aucune tokenisation à runtime. **Consigne (ajouté 2026-07-07, audit 07, finding 07-D1)** : sens anglais tant que la phrase n'est pas maîtrisée ; audio de la phrase (rejouable) dès que tous ses mots sont maîtrisés — dans un même combat, les deux consignes cohabitent donc naturellement selon l'âge de chaque phrase. L'audio est pré-généré (VOICEVOX, batch — étape 4bis du Pipeline) sur une **pool Disposition présélectionnée** dont la taille est chiffrée à la synthèse ; seules les phrases de cette pool sont éligibles au mode.
...
| `quests`             | quest_id, name, zone_ids[], steps[] (step_id, label, `target_zone_id` optionnel — *ajouté 2026-07-07, audit 08, finding 08-B14 : le marqueur ◎ « sur la cible active » n'avait aucune donnée de cible ; la zone suffit pour une carte-monde, pas de position tuile — cohérent avec l'indice du Journal « pas de position exacte »*)                                                                                                                                                                   |
...
4bis. **Audio de la pool Disposition** *(ajouté 2026-07-07, audit 07, finding 07-D1)* — présélection de la pool de phrases éligibles au mode Disposition (taille chiffrée à la synthèse), puis génération batch **VOICEVOX** (3-4 voix en rotation, comme l'étape 7bis) de l'audio de chaque phrase → `sentences.audio_ref` (nullable — seules les phrases de la pool en ont un ; sert de consigne aux phrases maîtrisées)
